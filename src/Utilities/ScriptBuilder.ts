...
export default class ScriptBuilder {
    ...
    getLatestModuleScript(moduleName: string): string {
        const command: string = `Get-Module -Name '${Utils.psEscapeSingleQuoted(moduleName)}' -ListAvailable | Sort-Object Version -Descending | Select-Object -First 1`;
        this.script += `try {
    ...
    checkModuleVersionScript(moduleName: string, version: string) {
        const command: string = `Get-Module -Name '${Utils.psEscapeSingleQuoted(moduleName)}' -ListAvailable | Where-Object Version -match '${Utils.psEscapeSingleQuoted(version)}'`;
        this.script += `try {
            $ErrorActionPreference = "Stop"
            $WarningPreference = "SilentlyContinue"
            $output = @{}
